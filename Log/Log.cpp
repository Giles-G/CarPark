...
// Asynchronous needs to set the length of the blocking queue, synchronous does not need to be set
bool Log::init(const char* file_name,
    ...
    if (max_queue_size >= 1) {
        ...
        pthread_t tid;
        // flush_log_thread is a callback function, which means creating a thread to write logs asynchronously
...
void Log::write_log(int level, const char* format, ...) {
    ...
    // Write a log, the maximum number of lines for m_count++, m_split_lines
    m_mutex.lock();
    m_count++;
    ...
    // The specific time content format for writing
